...
void loop() {
  ...
  if (distance < 500) {
    ...
    while (true) {
      trouverMonChemin();
      float nouvelleDistance = mesurerDistance();
      
      if (nouvelleDistance < 500) {
        avancer();
        break;
      }
    }
...
void regarderagauche() {
  myservo.write(30); // Angle de rotation pour regarder à gauche (à ajuster)
  delay(500);
}
//Tourner l ultrason a doite  
void regarderadroite() {
  myservo.write(150); // Angle de rotation pour regarder à droite (à ajuster)
  delay(500);
}
...
int VoieLibreAGauche() {
  ...
  delay(500); // Attente de positionnement
